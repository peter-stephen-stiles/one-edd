...
btnRefresh.addActionListener(ex->populateTables());
		...
		JScrollPane sclIndexes = new JScrollPane();
		JScrollPane sclConstraints = new JScrollPane();
	...
	private void selectTable() {
		if(tblTables.getSelectedRowCount()>0) {
			...
			if(dtot!=null) {
				...
				List<DTOColumn> list=sqlService.metaDataColumns(dtot.getTableCat(),dtot.getTableSchem(),dtot.getTableName());
