...
public class AddEquipmentScroll extends PDialog implements AddEquipmentI {

	private static final Logger LOGGER = LogManager.getLogger(MethodHandles.lookup().lookupClass());

	public AddEquipmentScroll(Window owner) {
		super(owner);
...
wm, wm.getName(), wm.getAC(),wm.getBaseMovement(),wm.getEncumberanceGP(),wm.getMagicBonus()
	...
	private void populateTable() {
		//
		ScrollService wms = (ScrollService) getDataService(Scroll.class);
		...
		for (Scroll wm : list) {
			Object[] row = new Object[] { wm, wm.getDescription() };
			tableData.add(row);
		}
	...
	private void selectCurrent() {
		try {
			...
			Scroll wm = (Scroll) tblEquipment.getModel().getValueAt(rowIndex, 0);
