...
public class AddEquipmentRanged extends PDialog implements AddEquipmentI {

	private static final Logger LOGGER = LogManager.getLogger(MethodHandles.lookup().lookupClass());

	public AddEquipmentRanged(Window owner) {
		super(owner);
		setModalityType(ModalityType.APPLICATION_MODAL);
		jbInit();
	}

	private EquipmentType type = EquipmentType.WEAPON_RANGED;
	...
	private final ColumnConfig[] equipmentConfigs = new ColumnConfig[] { new ColumnConfig("", 0, 0, 0),
			new ColumnConfig("Name", 20, 200, 5000), new ColumnConfig("Hands", 20, 150, 5000),
			new ColumnConfig("Range S", 20, 100, 5000), 
			new ColumnConfig("Range M", 20, 100, 5000), 
			new ColumnConfig("Range L", 20, 100, 5000), 
	...
	private void populateTable() {
		//
		WeaponRangedService wms = (WeaponRangedService) getDataService(WeaponRanged.class);
		...
		for (WeaponRanged wm : list) {
			Object[] row = new Object[] { wm, wm.getName(), wm.getRequiresHands(),wm.getRangeS(), wm.getRangeM(), wm.getRangeL(), wm.getSMDamage(), wm.getLDamage() };
			tableData.add(row);
		}
	...
	private void filter() {
		SwingUtilities.invokeLater(() -> {
			...
			if (filter.isEmpty()) {
				...
				for (Object[] oa : tableData) {
					String s = ((WeaponRanged) oa[0]).getName();
	...
	private void selectCurrent() {
		try {
			...
			WeaponRanged wm = (WeaponRanged) tblEquipment.getModel().getValueAt(rowIndex, 0);
