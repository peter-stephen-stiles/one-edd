...
public class AddEquipmentMagicRing extends PDialog implements AddEquipmentI {

	private static final Logger LOGGER = LogManager.getLogger(MethodHandles.lookup().lookupClass());

	public AddEquipmentMagicRing(Window owner) {
		super(owner);
...
wm, wm.getName(), wm.getAC(),wm.getBaseMovement(),wm.getEncumberanceGP(),wm.getMagicBonus()
	...
	private void populateTable() {
		//
		MagicRingService wms = (MagicRingService) getDataService(MagicRing.class);
		...
		for (MagicRing wm : list) {
			Object[] row = new Object[] { wm, wm.getName(), wm.getMagicalBonus() };
			tableData.add(row);
		}
	...
	private void filter() {
		SwingUtilities.invokeLater(() -> {
			...
			if (filter.isEmpty()) {
				...
				for (Object[] oa : tableData) {
					String s = ((MagicRing) oa[0]).getName();
	...
	private void selectCurrent() {
		try {
			...
			MagicRing wm = (MagicRing) tblEquipment.getModel().getValueAt(rowIndex, 0);
