...
public class AddEquipmentMiscellaneousItem extends PDialog implements AddEquipmentI {

	private static final Logger LOGGER = LogManager.getLogger(MethodHandles.lookup().lookupClass());

	public AddEquipmentMiscellaneousItem(Window owner) {
		super(owner);
		setModalityType(ModalityType.APPLICATION_MODAL);
		jbInit();
	}

	private EquipmentType type = EquipmentType.MISCELLANEOUS_ITEM;
	...
	private final ColumnConfig[] equipmentConfigs = new ColumnConfig[] { new ColumnConfig("", 0, 0, 0),
			...
			new ColumnConfig("Value", 20, 200, 150),
			};
	...
	private void populateTable() {
		//
		MiscellaneousItemService wms = (MiscellaneousItemService) getDataService(MiscellaneousItem.class);

		List<MiscellaneousItem> list;
		try {
		...
		for (MiscellaneousItem wm : list) {
			Object[] row = new Object[] { wm, wm.getName(),wm.getValueString()};
			tableData.add(row);
		}
	...
	private void filter() {
		SwingUtilities.invokeLater(() -> {
			...
			if (filter.isEmpty()) {
				...
				for (Object[] oa : tableData) {
					String s = ((MiscellaneousItem) oa[0]).getName();
	...
	private void selectCurrent() {
		try {
			...
			MiscellaneousItem wm = (MiscellaneousItem) tblEquipment.getModel().getValueAt(rowIndex, 0);
