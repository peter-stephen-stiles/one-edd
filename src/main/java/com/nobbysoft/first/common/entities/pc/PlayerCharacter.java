...
public class PlayerCharacter implements Serializable, DataDTOInterface<Integer> {
	...
	public PlayerCharacterLevel getFirstClassDetails() {
		PlayerCharacterLevel level = new PlayerCharacterLevel();
		level.setThisClass(firstClass);
		level.setThisClassExperience(firstClassExperience);
		level.setThisClassHp(firstClassHp);
		level.setThisClassLevel(firstClassLevel);
		return level;
	}

	public PlayerCharacterLevel getSecondClassDetails() {
		PlayerCharacterLevel level = new PlayerCharacterLevel();
		level.setThisClass(secondClass);
		level.setThisClassExperience(secondClassExperience);
		level.setThisClassHp(secondClassHp);
		level.setThisClassLevel(secondClassLevel);
		return level;
	}

	public PlayerCharacterLevel getThirdClassDetails() {
		PlayerCharacterLevel level = new PlayerCharacterLevel();
		level.setThisClass(thirdClass);
		level.setThisClassExperience(thirdClassExperience);
		level.setThisClassHp(thirdClassHp);
	...
	public void updateClassDetails(PlayerCharacterLevel details) {
		if(details.getThisClass().contentEquals(firstClass)) {
			...
			firstClassExperience = details.getThisClassExperience();
			firstClassHp = details.getThisClassHp();
			firstClassLevel = details.getThisClassLevel();
		} else if(details.getThisClass().contentEquals(secondClass)) {
			// update class details
			secondClassExperience = details.getThisClassExperience();
			secondClassHp = details.getThisClassHp();
			secondClassLevel = details.getThisClassLevel();
		} else if(details.getThisClass().contentEquals(thirdClass)) {
			// update class details
			thirdClassExperience = details.getThisClassExperience();
			thirdClassHp = details.getThisClassHp();
			thirdClassLevel = details.getThisClassLevel();
		} else {
			throw new IllegalStateException("Can't work out which class to update :( "+details.getThisClass());
		}
	}
	...
	public PlayerCharacterLevel[] getClassDetails() {
		return new PlayerCharacterLevel[] { getFirstClassDetails(), getSecondClassDetails(), getThirdClassDetails() };
	}
	...
	public int getHp() {
		return firstClassHp + secondClassHp + thirdClassHp;
	}

	public int getExperience() {
		return firstClassExperience + secondClassExperience + thirdClassExperience;
	}
	...
	public int[] getAtts() {
		return new int[] { attrStr, attrInt, attrWis, attrDex, attrCon, attrChr };
	}
	...
	public Object[] getAsRow() {
		return new Object[] { this, pcId, characterName, raceId, firstClass, gender.name(), alignment.name() };
	}
	...
	public String[] getColumnCodedListTypes() {
		return new String[] { null, null, Constants.CLI_RACE, Constants.CLI_CLASS, Constants.CLI_GENDER,
