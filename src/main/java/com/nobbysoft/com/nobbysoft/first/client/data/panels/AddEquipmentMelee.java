...
public class AddEquipmentMelee extends PDialog implements AddEquipmentI {

	private static final Logger LOGGER = LogManager.getLogger(MethodHandles.lookup().lookupClass());

	public AddEquipmentMelee(Window owner) {
		super(owner);
		setModalityType(ModalityType.APPLICATION_MODAL);
		jbInit();
	}

	private EquipmentType type = EquipmentType.MELEE_WEAPON;
	...
	private void populateTable() {
		...
		for (WeaponMelee wm : list) {
			Object[] row = new Object[] { wm, wm.getName(), wm.getRequiresHands(), wm.getSMDamage(), wm.getLDamage() };
			tableData.add(row);
		}
	...
	private void filter() {
		SwingUtilities.invokeLater(() -> {
			...
			if (filter.isEmpty()) {
				...
				for (Object[] oa : tableData) {
					String s = ((WeaponMelee) oa[0]).getName();
